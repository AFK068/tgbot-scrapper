...
import (
	...
	"github.com/AFK068/bot/internal/migration"
)
...
func (p *PostgresTestcontainer) SetupTestPostgresContainer(ctx context.Context) (*pgxpool.Pool, CleanFunc, error) {
	...
	if err := p.Migrate(logger.NewDiscardLogger()); err != nil {
		return nil, nil, err
	}
...
func (p *PostgresTestcontainer) Start(ctx context.Context) (int, error) {
	...
	port, err := p.Container.MappedPort(contextWithTimeout, nat.Port(p.Config.Storage.Port))
	if err != nil {
		return 0, err
	}
...
func (p *PostgresTestcontainer) Migrate(log *logger.Logger) error {
